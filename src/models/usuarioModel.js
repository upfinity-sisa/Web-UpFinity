...
function autenticar(email, senha) {
  ...
  var instrucaoSql = `
        SELECT idUsuario, nome, email, fkEmpresa, CPF FROM usuario WHERE email = '${email}' AND senha = '${senha}';
    `;
...
function cadastrarUsuario(nome, email, senha, cnpj, cpf, fkEmpresa) {
  ...
  return database.executar(selectSql).then((resultado) => {
    ...
    if (resultado.length > 0) {
      ...
      var instrucaoSql = `
        INSERT INTO Usuario (nome, email, senha, cpf ,fkEmpresa, fkTipo) VALUES ('${nome}','${email}', '${senha}', '${cpf}', '${fkEmpresa}', 1);
    `;
      console.log("Executando a instrução SQL: \n" + instrucaoSql);
