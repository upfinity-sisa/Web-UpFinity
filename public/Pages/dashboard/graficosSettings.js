...
function ultimasCapturas() {
  return fetch(`/dashboard/ultimas-capturas/3`, { cache: "no-store" })
    ...
    .catch((erro) => {
      console.error(
        `ultimasCapturas: erro na obtenção dos dados: ${erro.message}`
      );
  ...
  return fetch(`/dashboard/ultimos-horarios/3`, { cache: "no-store" })
    .then((response) => {
      if (response.ok) {
        ...
        console.error(
          "pegarUltimosHorariosCaptura: nenhum dado encontrado ou erro na API"
        );
      ...
      if (response.ok) {
        response.json().then((resposta) => {
          ...
          const totalUpTimePercent =
            (totalUpTimeSeg / totalSegundosSemana) * 100;
          const totalDownTimePercent =
            (totalDownTimeSeg / totalSegundosSemana) * 100;
        ...
        console.error(
          "carregarDowntime: nenhum dado encontrado ou erro na API"
        );
      }
    })
    .catch((erro) => {
      console.error(
        `carregarDowntime: erro na obtenção dos dados: ${erro.message}`
      );
    });
}
...
var chartLinha = new ApexCharts(
  document.querySelector("#caixagraflinha"),
  optionsComponentes
);
chartLinha.render();
